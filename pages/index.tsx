...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (fromQr) {
      ...
      localStorage.setItem('qrHuntCodesFound', JSON.stringify(Array.from(codesFound)))
    ...
    <Flex sx={{
      ...
      {fromQr && <>
        <Card sx={{
          ...
          <Paragraph variant='caption' sx={{ color: 'white', fontWeight: 'bold', pb: 1 }}>
            {codesRemaining > 0 ? 'It looks like you found one of the QR codes.' : 'You found all the QR codes!'}
          </Paragraph>

          <Paragraph variant='caption' sx={{ color: 'white' }}>
            {codesRemaining > 0 ? <span>There are <strong>{codesRemaining} more</strong> QR codes located around the school. Scan them all from the same phone and bring it to our next meeting to get some awesome stickers.</span>
            : `Bring this phone to our next Hack Club meeting and I'll give you some stickers!`}
          </Paragraph>
        </Card>
        <canvas ref={confettiCanvasRef} sx={{
          position: 'absolute',
          left: 0,
          top: 0,
          width: '100%',
