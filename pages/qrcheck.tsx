...
const QRCheck: React.FC = () => {
  ...
  const [state, setState] = useState<'found' | 'claimed' | 'notfound'>(
    'notfound'
  )
  ...
  return (
    <Layout center>
      ...
      >
        ...
        >
          {state === 'found' &&
            'Nice job! Show this page to Benjamin to claim your stickers.'}
          {state === 'claimed' && `Oops! You've already claimed your stickers.`}
          {state === 'notfound' &&
            `You haven't found all the QR codes. Keep looking around the school, or ask someone for a clue.`}
        </Paragraph>
      </Card>
    </Layout>
  )
